...
class _ProfileCreation2State extends State<ProfileCreation2> {
  ...
  // Methods
  void updateNickname(String value) => setState(() => nickname = value);

  void updateJobInterests(String value) => setState(() => jobInterests = value);

  void updateAddress(String value) => setState(() => address = value);

  void updateOccupation(String value) => setState(() => occupation = value);

  void updateUniversity(String value) => setState(() => university = value);

  void updateAboutYourself(String value) =>
      setState(() => aboutYourself = value);

  void updateHobbies(String value) => setState(() => hobbies = value);

  void updateLifestyleInterests(String value) =>
      setState(() => lifestyleInterests = value);

  void updateBio(String value) => setState(() => bio = value);

  void updateLocation(String value) => setState(() => selectedLocation = value);

  void updateSubject(String value) => setState(() => selectedReligion = value);

  // Dating Intentions methods
  void selectDatingIntention(String intention) {
    setState(() => selectedDatingIntention = intention);
  }

  void updateAgeFrom(double value) => setState(() => ageFrom = value.round());

  void updateAgeTo(double value) => setState(() => ageTo = value.round());

  void toggleStudent(bool? value) => setState(() => isStudent = value ?? false);

  void toggleWorking(bool? value) => setState(() => isWorking = value ?? false);

  void toggleOther(bool? value) => setState(() => isOther = value ?? false);
  ...
  void selectStatus(String status) => setState(() => selectedStatus = status);

  void selectSmoking(String smoking) =>
      setState(() => selectedSmoking = smoking);

  void selectDrinking(String drinking) =>
      setState(() => selectedDrinking = drinking);

  void selectClubs(String clubs) => setState(() => selectedClubs = clubs);

  void selectPetOwnership(String pet) => setState(() => ownsPet = pet);
                      ...
                      InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => VoicePromptRecordScreen(),
                            ),
                          );
                        },
                        child: SizedBox(
  ...
  int _getCurrentLength(String label) {
    switch (label) {
      ...
      case 'Lifestyle / Interests':
        return lifestyleInterests.length;
      default:
        return 0;
    }
  }
  ...
  Widget _buildDatingIntentionChip(String intention) {
    return GestureDetector(
      onTap: () => selectDatingIntention(intention),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 5),
        decoration: BoxDecoration(
          border: Border.all(color: Color(0xffFA6D8C)),
          color: selectedDatingIntention == intention
              ? Color(0xffFA6D8C)
              : Colors.white,
          borderRadius: BorderRadius.circular(20),
        ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: isLanguageSelected(language) ? Colors.green : Colors.white,
          borderRadius: BorderRadius.circular(15),
        ),
        child: Text(
          language,
          style: TextStyle(
            color: isLanguageSelected(language) ? Colors.white : Colors.black,
            fontSize: 12,
          ),
        ),
      ),
  ...
  Widget _buildStatusChip(String status) {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border.all(color: Color(0xffFA6D8C)),
          color: selectedStatus == status ? Color(0xffFA6D8C) : Colors.white,
          borderRadius: BorderRadius.circular(40),
        ),
        child: Text(
          status,
          style: TextStyle(
            color: selectedStatus == status ? Colors.white : Color(0xff000000),
            fontSize: 14,
            fontWeight: FontWeight.w500,
          ),
        ),
  ...
  Widget _buildLifestyleChip(String option, String category) {
    ...
    switch (category) {
      ...
      case 'drinking':
        ...
        break;
      case 'clubs':
        isSelected = selectedClubs == option;
        break;
    }
  ...
  Widget _buildRadioOption(String value, String label) {
    return Padding(
      ...
      child: Row(
        children: [
          GestureDetector(
            onTap: () => selectPetOwnership(value),
            child: Container(
              width: 20,
              height: 20,
              decoration: BoxDecoration(
                ...
                color: ownsPet == value ? Color(0xffFA6D8C) : Colors.white,
              ),
              child: ownsPet == value
